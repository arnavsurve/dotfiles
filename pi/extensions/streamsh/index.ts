...
async function roundTrip(socketPath: string, req: Envelope): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    sock.on("error", (err) =>
      reject(new Error(`streamsh daemon: ${err.message}`)),
    );
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      search: Type.Optional(
        Type.String({ description: "Fuzzy/substring search pattern" }),
      ),
      last_n: Type.Optional(
        Type.Number({ description: "Return the last N lines" }),
      ),
      cursor: Type.Optional(
        Type.Number({ description: "Start from this sequence number" }),
      ),
      count: Type.Optional(
        Type.Number({
      ...
      max_results: Type.Optional(
        Type.Number({ description: "Max search results (default 50)" }),
      ),
    }),
